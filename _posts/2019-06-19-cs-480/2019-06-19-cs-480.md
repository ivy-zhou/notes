...
- Bayesian Learning
  ...
  where $P(H\vert e)$ is the posterior distribution, $P(e\vert H)$ is the likelihood distribution, $P(H)$ is the prior probability, $P(e)$ evidence

  $$
  P(H\vert\pmb{e}) = kP(\pmb{e}\vert H)P(H)
  $$

- Suppose we want to make a prediction about a dataset query X:

  $$
  P(X\vert e) = \sum_i P(X\vert e, h_i)P(h_i\vert e)
  $$
...
- **Bayesian learning:** calculates probability of each hypothesis given all seen data
  ...
  $$
  P(h_i, \textbf{d}) = \alpha P(\textbf{d}\vert h_i)P(h_i)
  $$

  where **d** is seen data, pulled from **D**, all data

  $$
  P(X\vert\textbf{d}) = \sum_{i} P(X\vert\textbf{d}, h_i)P(h_i\vert\textbf{d}) = \sum_{i} P(X\vert h_i)P(h_i\vert\textbf{d})
  $$
...
- **likelihood:** probability of data under each hypothesis $P(\textbf{d} \vert h_i)$

  Assuming that observations are identically and independently distributed:

  $$
  P(\textbf{d} \vert h_i) = \prod_{j} P(d_j \vert h_i)
  $$
...
- **maximum a posteriori (MAP):** make predictions based on single maximum likelihood ($P(\textbf{d} \vert h_i)$) hypothesis, Bayesian learning with 1 term (no summation)
...
- **beta distribution:** $\text{beta}\[a, b\](\theta) = \alpha\theta^{a - 1}(1 - \theta)^{b - 1}$
  ...
  Suppose we observe a cherry candy and we have a hypothesis $\theta$. We can calulate the posterior distribution $P(\theta \vert D_1 = cherry)$, given that the prior distribution is a beta distribution.
